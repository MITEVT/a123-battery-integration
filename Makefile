...
# c files for testing
C_SRCS_TEST = $(wildcard $(patsubst %, %/*.$(C_EXT), . $(TEST_SRCS_DIRS))) src/charge.c src/balance.c src/ssm.c src/drain.c src/pack_manager.c ../lpc11cx4-library/evt_lib/src/brusa.c ../lpc11cx4-library/evt_lib/src/a123mbb.c
...
# flags for C compiler
C_FLAGS = -std=$(C_STD) -g -ggdb3 -fverbose-asm -Wa,-ahlms=$(OUT_DIR_F)$(notdir $(<:.$(C_EXT)=.lst)) -fstack-usage
