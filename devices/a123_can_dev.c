...
void a123_encode_bcm_cmd(bcm_cmd_msg_t *msg, uint8_t *buffer) {
	...
	buf += (msg->leakage_cmd & BCM_LEAKAGE_CMD_MASK) << BCM_LEAKAGE_CMD_SHIFT;
	buf += (msg->request_type & BCM_REQUEST_TYPE_MASK) << BCM_REQUEST_TYPE_SHIFT;
	buf += (msg->request_id & BCM_REQUEST_ID_MASK) << BCM_REQUEST_ID_SHIFT;
	buf += (msg->balance_target & BCM_BALANCE_TARGET_MASK) << BCM_BALANCE_TARGER_SHIFT;
